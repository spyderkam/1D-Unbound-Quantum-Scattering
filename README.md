...
It appears to be some polynomial function. Try fitting $V(x) = ax^3 + bx^2 + cx + d$ to the curve; I used Python's $\texttt{curve_fit}$ method from the $\texttt{scipy.optimize}$ library. This method uses non-linear least squares to fit a function [1].
